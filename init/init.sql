...
CREATE OR ALTER VIEW [dbo].[MBCount]
AS
WITH Info AS
(SELECT
	OrderList.EinkaufID,
...
FROM
	(SELECT
		EinkaufID,
		COUNT(ProduktID) AS NumberofProducts
	FROM Einkauf_Produkte
	GROUP BY EinkaufID
	HAVING COUNT(ProduktID) >= 2) AS OrderList
JOIN Einkauf_Produkte AS EP ON Orderlist.EinkaufID = EP.EinkaufID)

SELECT 
	Info1.ProduktID AS Product1,
	Info2.ProduktID AS Product2,
	COUNT(*) AS Frequency
FROM Info AS Info1
JOIN Info AS Info2 ON Info1.EinkaufID = Info2.EinkaufID
WHERE Info1.ProduktID != Info2.ProduktID 
  AND Info1.ProduktID < Info2.ProduktID
GROUP BY
	Info1.ProduktID,
	Info2.ProduktID;
GO

Create OR Alter View MBList AS
WITH Info AS
(SELECT
	OrderList.EinkaufID,
	EP.ProduktID
...
CREATE PROCEDURE MBCountName
AS
SELECT TOP (1000) P1.Produkt
      ,P2.Produkt
      ,[Frequency]
  FROM [Marketbasket].[dbo].[MBCount]
  Join Produkte AS P1 ON [Marketbasket].[dbo].[MBCount].[Product1] = P1.ID
  Join Produkte AS P2 ON [Marketbasket].[dbo].[MBCount].[Product2] = P2.ID
  Order By [Marketbasket].[dbo].[MBCount].[Frequency] DESC
GO
